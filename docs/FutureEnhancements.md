...
- ~~**Localizable strings**~~ — **Shipped in v3.0: Spanish (es-ES) + French (fr-FR)** — full in-app localization (String Catalog), localized App Store metadata + captioned screenshots. **v3.2 (planned): German (de) + Italian (it)** — full in-app + metadata, one bundled release, AI-only QA (back-translation checks, no native reviewer). Design: `docs/superpowers/specs/2026-07-24-localization-de-it-design.md`.
