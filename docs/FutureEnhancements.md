...
- ~~**Untranslated placement titles and "Main Library"**~~ — **Fixed for the next release (found 2026-09-16).** `Place Signature`, `Place Date` and `Main Library` shipped in English in every language through v3.7. All three were stored in a plain `String` and displayed later (`.navigationTitle(title)`, `Button(dest.name)`), and SwiftUI shows a `String` variable verbatim, so Xcode never harvested them and `verify-localization.py` check 5, which only sees `String(localized:)` calls, passed regardless. Now wrapped in `String(localized:comment:)` with es/fr/de/it translations; the check covers them. **The class of bug is still open:** any future UI text held in a `String` bypasses both. When passing UI text through a `String`, wrap it at the point of definition. QA: `docs/superpowers/qa/2026-09-16-de-it-placement-titles-back-translation.md`.
- ~~**Simplified Chinese (zh-Hans), in-app**~~ — **Built 2026-09-25, ships with the next release.** All 177 catalogue keys plus the camera and Face ID usage descriptions. **Deliberately in-app only:** no App Store metadata, no screenshots, no new territory. Two reasons. Mainland China needs an **ICP filing**, which requires a mainland-registered legal entity, so that storefront is closed to a solo Canadian developer regardless of translation. And the funnel data says a fifth language does not address the bottleneck: ~6 impressions/day, 1 first-time download in 90 days, so this was done for the users it serves rather than as a growth move. The product page's **Languages** row picks up "Chinese (Simplified)" automatically once the build ships, because that row is read from the `.lproj` folders in the bundle, not from App Store Connect — so the feature is advertised at zero metadata cost. A Chinese keyword field would need the store listing we chose not to add, and is the part least reviewable without a native speaker. QA: `docs/superpowers/qa/2026-09-25-zh-Hans-back-translation.md`. **Risk recorded there and worth repeating: nobody on this project can read the shipped text**, which is a step beyond the accepted de/it risk.
